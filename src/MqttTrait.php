...
trait MqttTrait
{
    public function mqttDecode($data, $server, $fd)
    {
        ...
        switch ($fix_header['type']) {
            case 1:  //CONNECT
                ...
                $authorized = \Yii::$app->auth->judge($connect_info);
                if ($authorized === false) {
    ...
    public function getConnectInfo($data)
    {
        ...
        $connect_info['will_retain'] = ($byte & 0x20 == 0x20);
        $connect_info['will_qos'] = ($byte & 0x18 >> 3);
        $connect_info['will_flag'] = ($byte & 0x04 == 0x04);
        $connect_info['clean_start'] = ($byte & 0x02 == 0x02);
        $hasUid = (($byte & 0x80) >> 7 == 0x01);
        $hasToken = (($byte & 0x40) >> 6 == 0x01);
        $offset += 1;
        $connect_info['keepalive'] = $this->decodeValue(substr($data, $offset, 2));
        $offset += 2;
        $connect_info['client_id'] = $this->decodeString(substr($data, $offset));
        $connect_info['uid'] = '';
        $connect_info['token'] = '';
        if ($hasUid && $hasToken) {
            $offset += strlen($connect_info['client_id']) + 2;
